...
def plot_mnistified_samples(dataset, class_names, conversion_fn, filename="", save_flag=False):
    ...
    fig.suptitle("MNISTified CIFAR10 examples", fontsize=16, fontweight='bold')
    fig.subplots_adjust(top=0.85)  # Add space between title and plots
    for i in range(10):
        axes[i].imshow(samples[i], cmap='gray')
        axes[i].axis('off')
        axes[i].set_title(class_names[i].upper(), fontsize=10, fontweight='bold')
        axes[i].set_xlabel(class_names[i], fontsize=8)
...
def add_distance_to_centroid(data_np, centroids):
    ...
    distances = np.zeros((data_np.shape[0], 1))
    
    for i in range(data_np.shape[0]):
        # Get the softmax prediction (columns index 0 to 9)
        softmax_pred = data_np[i, 0:10]
        
        # Get the class index from column 12 (index 11)
        class_index = int(data_np[i, 11])
        
        # Get the corresponding centroid
        centroid = centroids[class_index]
        
        # Calculate the Euclidean distance between the softmax prediction and the centroid
        distance = np.linalg.norm(softmax_pred - centroid)
        
        # Store the distance
        distances[i] = distance
    
    # Add the distances as a new column to data_np
    data_np = np.hstack((data_np, distances))
