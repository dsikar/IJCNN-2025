...
def add_distance_to_centroid(data_np, centroids):
    ...
    centroids : numpy.ndarray
        Array containing the centroids for each class.
    
    Returns:
    --------
    data_np_with_distances : numpy.ndarray
        Array with an additional column for the distance to the predicted class centroid.
    """
    # Create a copy of the data_np array
    data_np_with_distances = data_np.copy()
    
    # Add a column for the distance to the predicted class centroid
    distances = np.zeros(len(data_np))
    
    # Calculate the distance to the predicted class centroid for each row
    for i in range(len(data_np)):
        predicted_class = int(data_np[i, 11])
        softmax_output = data_np[i, :10]
        distances[i] = np.linalg.norm(softmax_output - centroids[predicted_class])
    
    # Add the distances as a new column
    data_np_with_distances = np.column_stack((data_np_with_distances, distances))
    
    return data_np_with_distances
